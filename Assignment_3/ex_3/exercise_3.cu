...
int main(int argc, char** argv)
{
    ...
    const int stream_size = num_particles / stream_count;
    ...
    for (int j = 0; j < num_iterations; j++) 
    {
        for (int i = 0; i < stream_count; i++) 
        {
            ...
            int num_blocks = (stream_size + block_size - 1) / block_size;
            cudaMemcpyAsync(&cu_particles[offset], &particles[offset], stream_size * sizeof(Particle), cudaMemcpyHostToDevice);
            cu_move_particles<<<num_blocks, block_size, 0, streams[i]>>>(cu_particles, stream_size, j, num_iterations);
            cudaMemcpyAsync(&particles[offset], &cu_particles[offset], stream_size * sizeof(Particle), cudaMemcpyDeviceToHost);
        }
    }
