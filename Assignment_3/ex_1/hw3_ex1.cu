...
/**
 * Applies a 3x3 convolution matrix to a pixel on the CPU.
 */
...
/**
 * Applies a 3x3 convolution matrix to a pixel on the GPU.
 */
 __device__
float gpu_applyFilter(float *image, int stride, float *matrix, int filter_dim)
{
    float pixel = 0.0f;
...
__global__ void gpu_gaussian(int width, int height, float *image, float *image_out)
{
    ...
    {
        int in_offset   = index_y * width + index_x;
        int out_offset  = (index_y + 1) * width + (index_x + 1);
        int sh_in_offset   = threadIdx.y * BLOCK_SIZE_SH + threadIdx.x; 
        
        sh_block[sh_in_offset] = image[in_offset];
        if (threadIdx.x == blockDim.x - 1) {
            sh_block[sh_in_offset + 1] = image[in_offset + 1];
            sh_block[sh_in_offset + 2] = image[in_offset + 2];
            if (threadIdx.y == blockDim.y - 1) {
                sh_block[sh_in_offset + 1 * BLOCK_SIZE_SH + 1] = image[in_offset + 1 * width + 1];
                sh_block[sh_in_offset + 1 * BLOCK_SIZE_SH + 2] = image[in_offset + 1 * width + 2];
                sh_block[sh_in_offset + 2 * BLOCK_SIZE_SH + 1] = image[in_offset + 2 * width + 1];
                sh_block[sh_in_offset + 2 * BLOCK_SIZE_SH + 2] = image[in_offset + 2 * width + 2];
            }
        }
        if (threadIdx.y == blockDim.y - 1) {
            sh_block[sh_in_offset + 1 * BLOCK_SIZE_SH] = image[in_offset + 1 * width];
            sh_block[sh_in_offset + 2 * BLOCK_SIZE_SH] = image[in_offset + 2 * width];
        }
        __syncthreads();

        image_out[out_offset] = gpu_applyFilter(&sh_block[sh_in_offset], BLOCK_SIZE_SH, gaussian, 3);
    }
}
...
__global__ void gpu_sobel(int width, int height, float *image, float *image_out)
{
    ...
    if (index_x < (width - 2) && index_y < (height - 2))
    {
        ...
        float gx = gpu_applyFilter(&sh_block[sh_in_offset], BLOCK_SIZE_SH, sobel_x, 3);
        float gy = gpu_applyFilter(&sh_block[sh_in_offset], BLOCK_SIZE_SH, sobel_y, 3);
        
        // Note: The output can be negative or exceed the max. color value
        // of 255. We compensate this afterwards while storing the file.
        image_out[out_offset] = sqrtf(gx * gx + gy * gy);
    }
}
