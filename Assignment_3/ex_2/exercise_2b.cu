...
int main(int argc, char** argv)
{
    ...
    Particle* unified_particles; 
    cudaMallocManaged(&unified_particles, buffer_size);
    for (int i = 0; i < num_particles; i++)
    {
        unified_particles[i].velocity = {0};
        unified_particles[i].position = {0};
    }
    ...
    {
        move_particles(unified_particles, num_particles, i, num_iterations);
    }
    ...
    {
        cu_move_particles<<<num_blocks, block_size>>>(unified_particles, num_particles, j, num_iterations);
    }
